...
app_main
 ...
 产品运行时只有一个音频 RX/TX owner（`audio_owner`）。Voice Runtime 是
  ...
  （BOOT 路径）。C4 一次会话多轮与 C5 恢复状态为 `HW-ACCEPTANCE-PENDING`
  （automatic verification: PASS）。Wake 经 Kconfig `VOICE_WAKE_SOURCE`
  接入 runtime；模型状态为 `WAKE MODEL PENDING`。BOOT/manual 与 mock wake
  分计，wake 不得计入 manual trigger。
```
